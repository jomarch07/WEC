...
'chemo.fit <- survfit(Surv(dtime,death)~chemo,data=dataRot)'
...
'grade.fit <- survfit(Surv(dtime,death)~grade,data=dataRot)'

#Tumor Stage fit
tumorstage.fit <- survfit(Surv(dataCanChemo$`Overall Survival (Months)`,dataCanChemo$`Patient's Vital Status`)~dataCanChemo$

'recur.fit <- survfit(Surv(dtime,death)~recur,data=dataRot)'1

meno.fit <- survfit(Surv(dtime,death)~meno,data=dataRot)
...
univ_models <- lapply(univ_formulas,function(x){coxph(x,data=dataRot)})
...
chemo.cox <- coxph(Surv(dtime,death) ~ chemo, data = dataRot)
summary(chemo.cox)
size.cox <- coxph(Surv(dtime,death) ~ size,data=dataRot)
